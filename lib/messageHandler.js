...
async function handleMessages(sock, messageUpdate) {
    try {
            ...
            if (fileSha256) {
                ...
                if (stickers[hash]) {
                    ...
                    if (foundCommand) {
                        ...
                        // Send processing reaction before execution
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            // Handle category buttons from menu
            if (buttonId.startsWith('cat_')) {
                const category = buttonId.replace('cat_', '');
                // Simulate calling the menu command with that category
                const args = [category];
                const context = { chatId, senderId, isGroup, channelInfo };
                const menuPlugin = require('../plugins/menu');
                await menuPlugin.handler(sock, message, args, context);
                return;
            }

            // Handle other buttons (channel, owner, support)
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, { 
                    text: '*Join our Channel:*\n[https://whatsapp.com/channel/0029VbCPnYf96H4SNehkev10](https://whatsapp.com/channel/0029VbCPnYf96H4SNehkev10)' 
                }, { quoted: message });
                return;
